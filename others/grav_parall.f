C     KAP/Digital_UA_F      4.0 k3011126 980529 o5r3so3  17-Jul-2000 13:16:25
# 1 "SPH_grav.F"
...
C!!!!!& LOCAL (NODE1)
       INTEGER II12
       PARAMETER (II12 = 1)
       II6 = MAXNODE1 - HI11 + II12
       II7 = (II6 + MPPNPR - II12) / MPPNPR
       II4 = HI11 + MPPID * II7
       II5 = MIN (MAXNODE1, II4 + (II7 - II12))
C!!!!! PARALLEL DO 
       DO NODE4=II4,II5
        PERMIT1(NODE4) = .TRUE.
       END DO
...
!      diagonal sumation:
       ...
       TRACE = 0.
       DO K=HI1,II1
        DIAG = DIAG + PS(HI1,K) * P_INERTIA(NODE,CELL,K,K) * PS(HI1,K)
...
!      triangular sumation: quadrupole is symmetric!
       ...
       DO L=II2,II3
        DO K=L+II2,II1
         TRIANG = TRIANG + PS(HI1,K) * P_INERTIA(NODE,CELL,K,L) * PS(HI1
     X     ,L)
        END DO
       END DO
...
C!!!!!& LOCAL (NODE1)
       INTEGER II26
       PARAMETER (II26 = 1)
       II20 = MAXNODE1 - HI11 + II26
       II21 = (II20 + MPPNPR - II26) / MPPNPR
       II18 = HI11 + MPPID * II21
       II19 = MIN (MAXNODE1, II18 + (II21 - II26))
C!!!!! PARALLEL DO 
       DO NODE4=II18,II19
        PERMIT1(NODE4) = .TRUE.
       END DO
...
!     diagonal sumation:
       ...
       TRACE = 0.
       DO K=HI1,II1
        DIAG = DIAG + D_X(HI1,K) * P_INERTIA(NODE,CELL,K,K) * D_X(HI1,K)
        TRACE = TRACE + P_INERTIA(NODE,CELL,K,K)
       END DO
!      triangular sumation: quadrupole is symetric!
       ...
       DO L=II2,II3
        DO K=L+II2,II1
         TRIANG = TRIANG + D_X(HI1,K) * P_INERTIA(NODE,CELL,K,L) * D_X(
     X     HI1,L)
        END DO
       END DO
...
!        singl gets the single internal-product:    ^q^ . ^p
        SINGL = 0.
        DO K=HI1,II1
         SINGL = SINGL + P_INERTIA(NODE,CELL,J,K) * D_X(HI1,K)
        END DO
        GNODE(KWSC,J) = GNODE(KWSC,J) + (SINGL * HI3 - D_X(HI1,J) * 
     X    TRACE) * RR3 + RR2 * D_X(HI1,J)
       END DO
       RETURN 
      END
