...
#        ifdef _BACKGROUND_FIELD_
          Bscal = Bscal + (Bbg(l)+B(i,l))*grad_w(i,j_,l) !Ok!
#        else
...
C    &           (rho(i)+0.0000005)
#    ifdef _BACKGROUND_FIELD_
      B_rate_k=(B_rate_k-(Bbg(k)+B(i,k))*div)/(rho(i)+0.0000005)
#    else
...
c
     ...
     &          (xp(j,l)-xp(k,l))
              ...
              eta_jk=eta_jk/(r2_jk*(m(j)+(m(k)))
              !to be optimized in future:
              for l=1,dim
                v_o(j,l)=v_o(j,l)-m(k)*(xp(j,l)-xp(k,l))*eta_jk
                v_o(k,l)=v_o(k,l)+m(j)*(xp(j,l)-xp(k,l))*eta_jk
              end do
            end do
          end do
      end do
     ...
     &          (xp(j,l)-xp(k,l))
              ...
              !to be optimized in future:
              for l=1,dim
                v(j,l)=v(j,l)-m(k)*(xp(j,l)-xp(k,l))*eta_jk
                v(k,l)=v(k,l)+m(j)*(xp(j,l)-xp(k,l))*eta_jk
              end do
            end do
          end do
      end do
